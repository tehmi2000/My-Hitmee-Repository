...
document.addEventListener("DOMContentLoaded", function(event){
    ...
    if (get_cookie('hitmee-username')){
        ...
        if(!sessionStorage.getItem("New") && window.speechSynthesis){
            window.speechSynthesis.speak(new SpeechSynthesisUtterance("welcome " + get_cookie("hitmee-username").value));
...
function getMyFriends() {
    fetch(`/api/${get_cookie("hitmee-username").value}/getFriends`).then(async function(response) {
        try {
...
const addNewUser = function (object) {
    ...
    // <div class="on_contact" id="on_contact">
    //     <img src="images/Waterfalls_USA_Crag_Trees_Hocking_Hills_State_Park_Ohio_Nature_10929x5553.jpg" alt="[temi's dp]" class="user_dp" onclick="Javascript:showImage(event)"/>
        
    //     <div class="cols display-container">
    //         <div class="rows">
    //             <span class="chat-name">HITMEE APP</span>
    //             <span class="display-time"><i>Time</i></span>
    //         </div>
    //         <div class="rows">
    //             <span class="chat-message">Dummy message...</span>
    //             <span class="unread-indicator"></span>
    //         </div>
    //     </div>

    //     <span class="pass-section">
    //         <button class="lock-toggle"><i class="icofont-lock"></i></button>
    //         <input type="password">
    //     </span>
    // </div>

    if(object.username != get_cookie('hitmee-username').value ){
        ...
        let div1=create("DIV"),
            ...
            div2=create("DIV"),
                div3=create("DIV"),
                    span1=createComponent("SPAN", object.username),
                    span2=createComponent("SPAN", 'active'),
                div4=create("DIV"),
                    span3=createComponent("SPAN", " "),
                    span4=createComponent("SPAN", no_of_unread_msg),
            span5 = create("SPAN"),
                button0 = create("button"),
                    i0 = create("i"),
                input0 = create("input");
        ...
        div1.classList.add("on_contact");
        div2.classList.add("cols", "display-container");
        div3.classList.add("rows");
        div4.classList.add("rows");
        span1.classList.add('chat-name');
        span2.classList.add('display-time');
        span3.classList.add('chat-message');
        span4.classList.add('unread-indicator', 'hide');
        span5.classList.add("pass-section");
        button0.classList.add("lock-toggle");
        i0.classList.add("icofont-lock");
        ...
        div2.onclick=function (event) {
            socket.emit('connectTo', event.currentTarget.id, get_cookie("hitmee-username").value);
        };
        ...
        socket.emit('getChat', get_cookie("hitmee-username").value, object.username);
    }
};
...
function search() {
    ...
    socket.emit('search_friend', get_cookie("hitmee-username").value, search_string);
}
        ...
        try{
            ...
            var div1=document.createElement("DIV");
                ...
                button.onclick=function (event) {
                    socket.emit('addBuddy', event.currentTarget.id, get_cookie("hitmee-username").value);
                };
...
function addFriend(username){
    socket.emit('addBuddy', username, get_cookie("hitmee-username").value)
}